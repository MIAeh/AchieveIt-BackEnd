...
public interface AuthorityMapper {
    ...
    @Select("SELECT members.*, users.username AS membername, users.usermail AS membermail FROM members, users, gitpermission WHERE (members.memberid = gitpermission.memberid AND users.userid = members.memberid AND members.projectid = #{projectID} AND members.deleted=false);")
    ...
    @Select("SELECT members.*, users.username AS membername, users.usermail AS membermail FROM members, users, mailpermission WHERE (members.memberid = mailpermission.memberid AND users.userid = members.memberid AND members.projectid = #{projectID} AND members.deleted=false);")
